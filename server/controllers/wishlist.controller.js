...
const getAllWishlistItemsHandler = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      message: "Could not retrieve wishlist items. Please try again later.",
    });
  }
};
  ...
  try {
    ...
    if (!wishlist.find((item) => item._id == productId)) {
      return res.status(400).json({
        message: "Product ID not found. No such item exists in wishlist.",
      });
    }
    ...
    return res.status(500).json({
      message: "Could not delete item from wishlist. Please try again later.",
    });
  }
};
